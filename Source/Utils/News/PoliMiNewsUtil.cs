...
public static class PoliMiNewsUtil
{
    ...
    private static int GetPolimiAuthorIdAndCreateItIfNotFound()
    {
        ...
        if (num > 0) return GetPolimiAuthorIdBecauseWeKnowItExists();

        Database.Database.Execute("INSERT INTO Authors (name,link) VALUES (@name,@link)", DbConfig.GetDbConfigNew(),
            new Dictionary<string, object?>
            {
                { "@name", PolimiName },
                { "@link", UrlPolimi }
