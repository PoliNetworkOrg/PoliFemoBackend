...
create function if not exists deleteUser(userid varchar(100)) returns int
BEGIN
...
create function if not exists deleteArticle(id int) returns int
BEGIN

    DECLARE
ct_en, ct_it INT;

SELECT content_en, content_it
INTO ct_en, ct_it
FROM Articles
WHERE article_id = id;
...
DELETE
FROM ArticleContent
WHERE ArticleContent.id IN (ct_en, ct_it);
...
create
event if not exists chores on schedule
    every '1' DAY
...
old users
    CREATE
TEMPORARY TABLE IF NOT EXISTS UsersToDelete(userid VARCHAR(100), days INT);
INSERT INTO UsersToDelete
SELECT user_id, expires_days
from Users
where DATEDIFF(NOW(), last_activity) > expires_days;
...
insert
ignore into Authors values(1, "Politecnico di Milano", "https://www.polimi.it/", "https://techcamp.polimi.it/wp-content/uploads/2018/11/LogoPolimi-bianco-h324.png");
insert
ignore into Tags values("ALTRO", null, null);
insert
ignore into Tags values("ATENEO", "https://www.coolinmilan.it/wp-content/uploads/2022/04/politecnico-milano-universita.jpg", "eWIYITE2%MWEt8F$Ipt8NHt8?wNHV@R*WB4;ofi^s.V@?boLnhoead");
insert
ignore into Tags values("RICERCA E INNOVAZIONE", "https://www.coolinmilan.it/wp-content/uploads/2022/04/politecnico-milano-universita.jpg", "eWIYITE2%MWEt8F$Ipt8NHt8?wNHV@R*WB4;ofi^s.V@?boLnhoead");
insert
ignore into Tags values("STUDENTI", "https://images.unsplash.com/photo-1543269865-cbf427effbad?ixlib=rb-4.0.3&auto=format&fit=crop", "eIH_rg.9t,Z#tm~Ut58_%2xuxtROR.g4xu.TE1xtRkxZyEM|xEkCae");
insert
ignore into Tags values("POLIMIWORLD", null, null);